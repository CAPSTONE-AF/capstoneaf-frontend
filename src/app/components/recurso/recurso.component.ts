...
export class RecursoComponent implements OnInit {
  ...
  constructor(private recursoService: RecursoService,private route:ActivatedRoute,private router:Router,
  ...
  ngOnInit(): void {
    this.route.queryParams
      .subscribe(params => {
        ...
        this.tituloTema = params.tituloTema;
      });
  ...
  public getRecursos(nombreCurso: string,tituloTema: string,showNotification: boolean): void {
    this.recursoService.getRecursos(nombreCurso,tituloTema).subscribe(
      (response: Recurso[]) => {
        this.recursos = response;
        if (showNotification) {
          this.sendNotification(NotificationType.SUCCESS, `${response.length} recurso(s) cargado(s) exitosamente.`);
        }
      },
...
public onAddNewRecurso(recursoForm: NgForm): void {
  ...
  const formData = this.recursoService.createRecursoFormDate(this.nombreCurso, this.tituloTema, null, recursoForm.value);
    this.recursoService.addRecurso(formData).subscribe(
      (response: Recurso) => {
        ...
        recursoForm.reset();
        this.sendNotification(NotificationType.SUCCESS, `Recurso: ${response.nombre} registrado exitosamente`);
      },
      (errorResponse: HttpErrorResponse) => {
        this.sendNotification(NotificationType.ERROR, errorResponse.error.message);
      }
...
public onUpdateRecurso(): void {
  const formData = this.recursoService.createRecursoFormDate(this.nombreCurso, this.tituloTema, this.currentNombre, this.editRecurso);
    this.recursoService.updateRecurso(formData).subscribe(
      (response: Recurso) => {
        this.clickButton('closeEditRecursoModalButton');
        this.getRecursos(this.nombreCurso,this.tituloTema,false);
        this.sendNotification(NotificationType.SUCCESS, `Recurso: ${response.nombre} actualizado exitosamente`);
      },
      (errorResponse: HttpErrorResponse) => {
        this.sendNotification(NotificationType.ERROR, errorResponse.error.message);
      }
...
public onDeleteRecurso(nombre: string,): void {
  this.recursoService.deleteRecurso(this.nombreCurso,this.tituloTema,nombre).subscribe(
