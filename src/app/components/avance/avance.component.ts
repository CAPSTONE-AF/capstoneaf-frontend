...
export class AvanceComponent implements OnInit {
  ...
  constructor(private avanceService: AvanceService, private temaService: TemaService,private router:Router,
  ...
  public getAvances(showNotification: boolean): void {
    this.avanceService.getAllAvancesByUserId(this.user.id).subscribe(
      (response: Avance[]) => {
        this.avances = response;
      }
    );
}

private convertirFormatoDeFecha(fechaPorConvertir: Date): string {
  return this.datepipe.transform(fechaPorConvertir, 'yyyy/MM/dd HH:mm a');
}
